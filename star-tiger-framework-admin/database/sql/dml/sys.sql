...
values (replace(uuid(), '-', ''), '查询菜单', '/sys/menu/**', 'GET'),
       ...
       (replace(uuid(), '-', ''), '启用停用菜单', '/sys/menu/enableStatus/**', 'PUT'),
       ...
       (replace(uuid(), '-', ''), '启用停用资源', '/sys/resource/enableStatus/**', 'PUT'),
       ...
       (replace(uuid(), '-', ''), '锁定解锁用户', '/sys/user/lock/**', 'PUT'),
       (replace(uuid(), '-', ''), '重置密码', '/sys/user/resetPassword/*', 'PUT');
commit;
